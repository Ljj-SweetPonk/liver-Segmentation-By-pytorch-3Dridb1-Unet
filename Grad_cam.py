...
if __name__ == '__main__':
    model_path='logs/MSAA_Epoch100-Total__Loss0.0763.pth'
    ...
    output_dir='cam'
    classes = ('liver','background')
    fmap_block = list()
    grad_block = list()

    # 图片读取；网络加载
#   img = cv2.imread(path_img, 1)  # H*W*C
    img = cv2.imread('Livers/Images/l__image_64.png', 1)  # H*W*C
    print(img.shape)#(512,512,3)
    img_input = img_preprocess(img)
    print(img_input.shape)#(torch.Size([1,3,512,512])
    net = mt4.UNet4(3, 2)
    # net = Net()
    ...
    print('图片已经经过模型处理过后了：', output.shape)
    print('把图片变成numpy然后一顿操作：。。。。。。')
    idx = np.argmax(output.cpu().data.numpy())#,axis=
    ...
    # 保存cam图片
    img_show = np.float32(cv2.resize(img, (512, 512))) / 255
